...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos):
    ...
    # Handle adaptive cruise control button events
    self._handle_adaptive_cruise_buttons(CS, can_sends)
  ...
  def _handle_adaptive_cruise_buttons(self, CS, can_sends):
    """
    Handle adaptive cruise control button events generated by the CarState
    """
    from cereal import car
    ButtonType = car.CarState.ButtonEvent.Type
    
    # Check for setCruise button events from adaptive cruise logic
    if hasattr(CS.out, 'buttonEvents'):
      for button_event in CS.out.buttonEvents:
        if button_event.type == ButtonType.setCruise and not button_event.pressed:
          # Send cruise activation command
          can_sends.append(fiatcan.create_cruise_buttons(
            self.packer_adas, 
            self.cruise_button_counter, 
            activate=True
          ))
          self.cruise_button_counter += 1
